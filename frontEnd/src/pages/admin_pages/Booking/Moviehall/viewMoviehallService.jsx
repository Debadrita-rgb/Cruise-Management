...
const ViewMoviehallService = () => {
  ...
  const handleAddClick = (category) => {
    ...
    if (!token || token === "") {
      ...
      navigate(
        `/admin/booking/addMoviehallService?category=${encodeURIComponent(
          category
        )}`
      );
    }
  ...
  const handleCategoryClick = async (categoryName) => {
    ...
    try {
      ...
      console.error("Error fetching Moviehall services:", error);
    } finally {
  ...
  const filteredItems = categoryItems
    ...
    .map((item, index) => ({
      ...
      editPath: `/admin/booking/editMoviehallService/${item._id}`,
    }));
...
export default ViewMoviehallService;
