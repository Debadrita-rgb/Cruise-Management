...
const ViewSalonService = () => {
  ...
  const handleAddClick = (category) => {
    ...
    if (!token || token === "") {
      ...
      navigate(
        `/admin/booking/addSalonService?category=${encodeURIComponent(
          category
        )}`
      );
    }
  ...
  const filteredItems = categoryItems
    ...
    .map((item, index) => ({
      ...
      editPath: `/admin/booking/editSalonService/${item._id}`,
    }));
        ...
        selectedCategory && (
          <div>
            ...
            <TableComponent
              ...
              columns={["Id", "ServiceName", "Price", "ServiceTime"]}
              data={filteredItems}
...
export default ViewSalonService;
