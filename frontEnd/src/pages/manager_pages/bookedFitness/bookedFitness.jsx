...
const BookedFitness = () => {
  ...
  const [fitnessBookings, setFitnessBookings] = useState([]);
    const [acceptedFitness, setAcceptedFitness] = useState([]);

  useEffect(() => {
    const token = localStorage.getItem("token");
    fetch("http://localhost:5000/manager/get-booked-fitness", {
      headers: {
      ...
      .then((data) => {
        setFitnessBookings(data.bookings || []);
        setAcceptedFitness(data.accepted || []);
      })
  ...
  const acceptOrder = async (id) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/manager/accept-fitness-order/${id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        ...
        setFitnessBookings((prev) => prev.filter((order) => order._id !== id));

        // Add to accepted list
        const acceptedOrder = fitnessBookings.find((order) => order._id === id);
        if (acceptedOrder) {
          setAcceptedFitness((prev) => [
            ...prev,
  ...
  const filteredOrders =
    activeTab === "Booking" ? fitnessBookings : acceptedFitness;

  // const filteredOrders = fitness.filter(
  //   (order) =>
