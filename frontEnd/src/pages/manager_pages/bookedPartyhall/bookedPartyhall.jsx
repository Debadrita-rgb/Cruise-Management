...
const BookedPartyhall = () => {
  ...
  const [activeTab, setActiveTab] = useState("Booking"); // 'Booking' or 'Accepted'
  const navigate = useNavigate();
const [partyhallBookings, setPartyhallBookings] = useState([]);
  const [acceptedPartyhall, setAcceptedPartyhall] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/manager/get-booked-partyhall", {
      ...
      .then((data) => {
        setPartyhallBookings(data.bookings || []);
        setAcceptedPartyhall(data.accepted || []);
      })
  ...
  const acceptOrder = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPartyhallBookings((prev) =>
          prev.filter((order) => order._id !== id)
        );

        // Add to accepted list
        const acceptedOrder = partyhallBookings.find(
          (order) => order._id === id
        );
        if (acceptedOrder) {
          setAcceptedPartyhall((prev) => [
            ...prev,
  ...
  // const filteredOrders = partyhall.filter(
  //   (order) =>
  //     order.status === (activeTab === "Booking" ? "Booking" : "Accepted")
  // );
  const filteredOrders =
    activeTab === "Booking" ? partyhallBookings : acceptedPartyhall;
              ...
              <>
                ...
                <div className="flex w-full justify-between">
                  ...
                  <div className="w-1/2 pl-6">
                    ...
                    <p className="text-sm text-gray-500">
                      <strong>Start Time:</strong> {order.details.startTime}
                    </p>
                    <p className="text-sm text-gray-500">
                      <strong>End Time:</strong> {order.details.endTime}
                    </p>
                    <p className="text-sm text-gray-500">
                      <strong>Hall Name:</strong> {order.hallName || "N/A"}
                    </p>
                  </div>
                  </div>
                  <FaChevronUp
...
export default BookedPartyhall;
